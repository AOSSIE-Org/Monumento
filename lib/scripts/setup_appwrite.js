...
async function setupAppwrite() {
  try {
    ...
    // Create a new database with unique identifier
...
async function addSampleMonumentData(databases, databaseId) {
  try {
    ...
    // Retrieve the created collections
    const collections = await databases.listCollections(databaseId);
    console.log('Collections retrieved:', collections.total);

    let monumentsCollection = null;
    let localExpertsCollection = null;

    // Find the specific collections by name
    for (const collection of collections.collections) {
      if (collection.name === 'monuments') {
        monumentsCollection = collection;
      }
      if (collection.name === 'localExperts') {
        localExpertsCollection = collection;
      }
    }

    if (!monumentsCollection) {
      throw new Error('Monuments collection not found');
    }

    if (!localExpertsCollection) {
      throw new Error('Local experts collection not found');
    }

    console.log('Found monuments collection ID:', monumentsCollection.$id);
    console.log('Found localExperts collection ID:', localExpertsCollection.$id);
