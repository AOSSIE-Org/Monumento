...
async function setupAppwrite() {
  try {
    ...
    let repoUrl = await getInput('Enter Git repository URL (press Enter to use default): ');

    if (!repoUrl) {
      repoUrl = "https://github.com/HelloSniperMonkey/reset_and_verifyemail_node_appwrite.git";
      console.log(`Using default repository: ${repoUrl}`);
    }
    ...
    await createAppwriteFunction(Functions,projectId, repoUrl);
...
async function createAppwriteFunction(Functions,projectId, repoUrl) {
  try {
    // Function details
